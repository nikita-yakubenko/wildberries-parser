...
class WildberriesParser:
	...
	def get_products_by_seller(self, seller_id: int, max_products: int, ordering: str) -> list:
		...
		results = asyncio.run(self.get_products_by_seller_async(seller_id=seller_id, max_pages=max_pages, ordering=ordering))
		products = []
	...
	async def get_full_product_data(self, product_id):
		...
		product_data_dict = {'main':main.result(), 'detail': detail.result(), 'prices': prices.result(), 'sold_count': sold_count.result(), 'seller': seller.result(), 'image': image.result()}
